...
class AppStore:
    ...
    def _init(self):
        with self._lock, self._conn() as conn:
            ...
            row = conn.execute("SELECT value FROM app_meta WHERE key='quality_schema'").fetchone()
            if not row or row["value"] != 'v3-core8':
                conn.execute("DELETE FROM contact_quality")
                conn.execute("UPDATE quality_state SET ready=0,running=0,updated_at=0,error=NULL WHERE id=1")
                conn.execute("INSERT OR REPLACE INTO app_meta(key,value) VALUES('quality_schema','v3-core8')")
